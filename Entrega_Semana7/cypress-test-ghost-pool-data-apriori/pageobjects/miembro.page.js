...
const fs = require("fs");
//let rawdata = fs.readFileSync("MOCK_DATA_MEMBERS.json");
//let rawdata
//let datapoolapriori = JSON.parse(rawdata);
let datapool = [];
...
class MiembroPage {
    ...
    constructor(){
        //rawdata = fs.readFile("MOCK_DATA_MEMBERS.json");
        //datapool = JSON.parse(rawdata);
    }
    ...
    setValorBuscado(valor) { this.getInputBuscar().type(valor); cy.wait(3000); }
    ...
    verificaCreacionMiembro(obj){
        cy.get('body').then(($body)=>{
            if ($body.find('svg[class="retry_svg__retry-animated"]').length){
               console.log("Tupla de Datos Miembro NO aceptada: ", obj)
    ...
    crearMiembroObj(objMember){
        this.getMenuMiembro().click()
        this.getNuevoMiembro().click()
        this.getNombreMiembro().type(objMember.nombre, {force: true})
        this.getEmailMiembro().clear({force: true})
        this.getEmailMiembro().type(objMember.email, {force: true})
        this.getLabelMiembro().type(objMember.label+"\n", {force: true})
        this.getNotaMiembro().type(objMember.nota, {force: true})
    ...
    crearMiembro(nombre, email, label, nota){
        ...
        this.getLabelMiembro().type(label+"\n", {force: true})
        this.getNotaMiembro().type(nota, {force: true})
        this.getBtnSalvarMiembro().click()
        cy.wait(3000)
    }    

    crearMiembroConFoto(nombre, email, label, nota, carpeta, nombreFoto, sufijo){
        this.getMenuMiembro().click()
        this.getNuevoMiembro().click()
        this.getNombreMiembro().type(nombre, {force: true})
        this.getEmailMiembro().type(email, {force: true})
        ...
        this.getBtnSalvarMiembro().click()
        cy.wait(3000)
        cy.screenshot(carpeta+"/"+nombreFoto+sufijo)
        cy.wait(2000)
    }

    editarMiembroObj(objMember){
    ...
    cambiarLabel(correo, label){
        this.getMenuMiembro().click()
        this.getInputBuscar().clear()
        this.setValorBuscado(correo+'\n')
        this.getPrimeraFilaMiembro().click()
        this.getLabelMiembro().clear({force: true})
        this.getLabelMiembro().type('{selectall}{backspace}{backspace}'+label+"\n", {force: true})
        this.getBtnSalvarMiembro().click()
        cy.wait(2000)
    }
