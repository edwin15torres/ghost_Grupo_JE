...
async function executeTest(){
    ...
    let resultInfo = {}
    let datetime = new Date().toISOString().replace(/:/g,".");
    fs.readdir(dirCypGhost3_4, (err, files) => {
      //console.log(files)
      files.forEach((file) => {
        if (file != undefined) {
          fs.readdir(dirCypGhost3_4 + "/" + file, (err, files2) => {
            //console.log(files2)
            if (!fs.existsSync(`./results/Cypress/${nombrever3_41}/_compare_/${nombrever4_41}/${file}`)){
              fs.mkdirSync(`./results/Cypress/${nombrever3_41}_compare_${nombrever4_41}/${file}`, { recursive: true });
            }
            html = ""
            files2.forEach((file2) => {
              valida(dirCypGhost3_4, file, file2, dirCypGhost4_41)
            })
            fs.copyFileSync('./index.css', `./results/Cypress/${nombrever3_41}_compare_${nombrever4_41}/${file}/index.css`)
          })
        }
      })
    })
...
async function valida(dir1, subdir, archivo, dir2) {
  ...
  fs.writeFileSync(`./results/Cypress/${nombrever3_41}_compare_${nombrever4_41}/${subdir}/${archivo}`, data.getBuffer());
  
  html = html + browser(
    datetime,
    resultInfo,
    `../../${nombrever3_41}/${subdir}/${archivo}`,
    `../../${nombrever4_41}/${subdir}/${archivo}`,
    `./${archivo}`
  );
  
  fs.writeFileSync(`./results/Cypress/${nombrever3_41}_compare_${nombrever4_41}/${subdir}/report.html`, createReport(datetime, html));
