...
describe("Test creating a tag in Ghost", () => {
  beforeEach(() => {
    ...
    it("Create a tag", () => {
      ...
      if(appVersion == "3.41.1") cy.get(".ember-view.gh-btn.gh-btn-green").click();
      else cy.get(".ember-view.gh-btn.gh-btn-primary").click();
      cy.wait(2000);
      cy.screenshot(`step_3/${appReference}-create-tag`);
      cy.get("#tag-name").click({force: true});
      cy.get("#tag-name").type("Tag1",{force: true});
      if(appVersion == "3.41.1") cy.get(".gh-btn.gh-btn-blue.gh-btn-icon.ember-view").click();
      else cy.get(".gh-btn.gh-btn-primary.gh-btn-icon.ember-view").click();   
      cy.wait(2000);
      cy.location().should((loc) => {
        expect(loc.href).to.include('/tag1')
      })
    });
});
